...
const toggleVideoLike = asynchandler(async (req, res) => {
  ...
  if (alreadyliked) {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { isliked: false }, "toogled like"));
  }
  await Like.create({
    Video: videoId,
    likedBy: req.user?._id,
...
const getLikedVideos = asynchandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, likedVideos, "All Video fetched"));
  } catch (error) {
    return res.status(500).json(new ApiError(500,"some error occur"));
